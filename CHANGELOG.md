...
### 改进 / Improvements
- 🎨 降低单个折叠按钮的默认透明度
  - 非 hover 状态下的 opacity 从 0.5 降低到 0.2
  - 按钮更不显眼，只在鼠标悬停时清晰显示
  - 提供更清爽的阅读体验
...
### 修复 / Fixed
- 🐛 修正 Grok 只在 AI 回答上显示折叠按钮
  - 将 `answerContainerSelector` 从 `.message-bubble` 改为 `.items-start .message-bubble`
  - 通过父元素的 `items-start` 类区分 AI 回答（items-start）和用户消息（items-end）
  - 现在折叠按钮只出现在 AI 回答上，不会出现在用户问题上
...
- 🎨 优化 Grok 全局按钮位置
  - 全局按钮现在显示在页面顶部工具栏
  - 更新 `globalControlsSelector` 为 `.h-16.top-0.absolute.z-10.flex.flex-row`
  - 添加专门的插入逻辑，将按钮插入到右侧控制区域之前
  - 优化按钮样式以匹配 Grok 的界面风格
...
### 修复 / Fixed
- 🐛 修正 Grok.com 选择器配置（基于实际页面结构）
  - `answerContainerSelector`: `[data-testid*="message"]` → `.message-bubble`
  - `answerContentSelector`: 保持 `.prose, [class*="markdown"]`
  - `globalControlsSelector`: `header` → `.action-buttons` (Grok 没有 header，使用操作按钮区域)
  - 更新 CSS 样式以匹配实际的类名
...
### 新增功能 / Added
- ✨ 新增对 Grok.com 的初步支持（实验性）
  - 添加了 Grok 的基础配置
  - 添加了 manifest 权限配置
  - 添加了初始的 CSS 样式
  - ⚠️ **选择器需要在实际页面上验证和调整**
  - 提供了诊断脚本 `debug/grok_diagnostic_script.js`
